		...
		<table width="100%" border="0" cellpadding="0" cellspacing="0">
			<tbody><tr>
				<td style="width: 85%;">
					...
					<?else:?>
						<input class="search-query input-search-main" type="text" name="q" value="<?=$arResult["REQUEST"]["QUERY"]?>" />
					<?endif;?>
				</td>
				<td>
					&nbsp;
				...
				<td align="right">
					<input class="search-button btn_search" type="submit" value="<?echo GetMessage("CT_BSP_GO")?>" />
				</td>
			</tr>
		</tbody></table>
...
<?if(isset($arResult["REQUEST"]["ORIGINAL_QUERY"])):
	...
	<?elseif(count($arResult["SEARCH"])>0):?>
		<?if($arParams["DISPLAY_TOP_PAGER"] != "N" && $arResult["NAV_RESULT"]->SelectedRowsCount() > 3) echo $arResult["NAV_STRING"]?>
		<?foreach($arResult["SEARCH"] as $arItem):?>
			<div class="catalog_item">				
                <div class="catalog_item_info" style="height: 320px;">                    
                    <a class="catalog_item_img" href="<?echo $arItem["URL"]?>">
                        <?if($arItem['IMAGE']['SRC']):?>
                            <img alt="<?=str_replace(array('<b>', '</b>'), array("", ""), $arItem["TITLE_FORMATED"])?>" src="<?=$arItem['IMAGE']['SRC']?>">
                        <?else:?>
                            <img width="150" src="/img/logo.png">
                        <?endif;?>
                    </a>                    
                    <a class="catalog_item_name" href="<?echo $arItem["URL"]?>"><?echo $arItem["TITLE_FORMATED"]?></a>
                    <div class="catalog_item_properties">
                        <div class="catalog_item_prop"><?echo $arItem["BODY_FORMATED"]?></div>                      					
                    </div>
                </div>
            </div>
		<?endforeach;?>
		<?if($arParams["DISPLAY_BOTTOM_PAGER"] != "N" && $arResult["NAV_RESULT"]->SelectedRowsCount() > 3) echo $arResult["NAV_STRING"]?>
