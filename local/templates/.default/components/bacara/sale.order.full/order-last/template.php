        ...
        <? if ($arResult["CurrentStep"] == 2) { ?>
            <div class='span3 left_part'>
                <div class="section_nav">
                        ...
                        <? foreach ($arResult['PAYSYSTEM'] as $key => $pay) { ?>
                            <div class="switch_control" id="pay_<?= $pay['ID'] ?>">
                                <label><input type="radio"
                                              <? if ($pay['ID'] != 1 && !in_array(12, $USER->GetUserGroupArray())) { ?>disabled<? } ?>
                                              <? if ($key == 0){ ?>CHECKED<? } ?> name="PAY_SYSTEM_ID"
                    ...
                    <div>
                        <p>Для того, чтобы оплатить заказ с помощью банковской карты, <span class="call-operator">необходимо связаться с оператором</span>
                            и убедиться в наличии заказываемых позиций.</p>
                    </div>
                </div>
                <? foreach ($arResult['PAYSYSTEM'] as $key => $pay) { ?>
    ...
    <? } ?>
                ...
                if ($arOrderProps["TYPE"] == "MULTISELECT") {
                    if (count($arResult["POST"]["ORDER_PROP_" . $arOrderProps["ID"]]) > 0) {
                        for ($i = 0; $i < count($arResult["POST"]["ORDER_PROP_" . $arOrderProps["ID"]]); $i++) {
                            ?><input type="hidden" name="ORDER_PROP_<?= $arOrderProps["ID"] ?>[]"
                                     value="<?= $arResult["POST"]["ORDER_PROP_" . $arOrderProps["ID"]][$i] ?>"><?
                        }
                    } else {
                    ...
                    ?><input type="hidden" name="ORDER_PROP_<?= $arOrderProps["ID"] ?>"
                             value="<?= $arResult["POST"]["ORDER_PROP_" . $arOrderProps["ID"]] ?>"><?
                }
            }
            ?>
        <?endif ?>
        ...
        <? if ($arResult["CurrentStep"] > 3 && $arResult["CurrentStep"] < 6):?>
            <input type="hidden" name="DELIVERY_ID"
                   value="<?= is_array($arResult["DELIVERY_ID"]) ? implode(":", $arResult["DELIVERY_ID"]) : IntVal($arResult["DELIVERY_ID"]) ?>">
        <?endif ?>
...
<?if(!$arResult['CONFIRMED'] ):?>
    <script>
        $(document).ready(function () {
            $(".confirm_ofert").on('click', function () {
                $.ajax({
                    type: "POST",
                    url: '/ajax/confirm_ofert.php',
                    data: {
                        ofert_id:$(this).attr('data')
                    }
                });
            })
        })
