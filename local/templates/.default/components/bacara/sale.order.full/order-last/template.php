...
else
{
	...
	<div class='span3 left_part'> 
		<div class="section_nav">
			<div class="pay_type">
				...
				<?foreach($arResult['PAYSYSTEM'] as $key=>$pay) { ?>
					<div class="switch_control" id="pay_<?=$pay['ID']?>">
						<label><input type="radio" <?if($pay['ID'] != 1 && !in_array(12, $USER->GetUserGroupArray())) {?>disabled<?}?> <?if($key==0){?>CHECKED<?}?> name="PAY_SYSTEM_ID" value="<?=$pay['ID']?>"><?=$pay['NAME']?></label>
					</div>				
				<? } ?>
            ...
            <div>
                <p>Для того, чтобы оплатить заказ с помощью банковской карты, <span class="call-operator">необходимо связаться с оператором</span> и убедиться в наличии заказываемых позиций.</p>
            </div>
		</div>	
		<?foreach($arResult['PAYSYSTEM'] as $key=>$pay) { ?>		
