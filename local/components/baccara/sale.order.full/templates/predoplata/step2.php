...
if (!function_exists('PrintPropsForm'))
{
	function PrintPropsForm($arSource=Array(), $PRINT_TITLE = "", $arParams)
	{
		...
		$adr = $arUser['UF_ADDRESS_REG'];
		if(strlen($adr) == 0)
		 $adr = $arUser['UF_ADDRESS_POST'];
		...
		if (!empty($arSource))
		{
			...
			{
				if($arProperties['FIELD_NAME'] == 'ORDER_PROP_4' && strlen($arProperties["VALUE"]) == 0) { 
					$arProperties["VALUE"] = $arUser['PERSONAL_PHONE'];
				} elseif($arProperties['FIELD_NAME'] == 'ORDER_PROP_3' && strlen($arProperties["VALUE"]) == 0) { 
					$arProperties["VALUE"] = $adr;
				}
				...
				{
					?>
					<div class="form_section_title"><?= 'Ваши данные' ?>:</div>
					<?
				}
				?>
				<div class="input_group <?=$arProperties['FIELD_NAME']?>">
...
if(!empty($arResult["USER_PROFILES"]))
{
	...
	?>
	<div class="order_form" style="padding:20px 0;">
...
<div  style="margin-top:0px;" class="switch_content deliver_form profile_new active">
    <div class="form_row">
        <div class="dual_col">
...
?>
        ...
        <div style="display:none;" class="dual_col deliver_types">	
            <div class="input_group">
                <input type="radio" id="d2" value="2" name="DELIVERY_ID" CHECKED>
            </div>
        </div>
        <div class="clr"></div>
    </div>
...
<script>
	...
	function hide_dostavka(d_id) { 
		if(d_id == 2) { 
			$(".ORDER_PROP_3, .ORDER_PROP_2").slideUp();
		} else { 
			$(".ORDER_PROP_3, .ORDER_PROP_2").slideDown();
		}
	}
	...
	$(function(){
		...
		var delivery_inpt = $("input[name=DELIVERY_ID]:checked").val();
		hide_dostavka(delivery_inpt);
...
<div class="order_complete">
	<input type="submit" class="button" name="contButton" value="подтвердить">
</div>
