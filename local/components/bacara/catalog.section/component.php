...
if($this->StartResultCache(false, array($arrFilter, ($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()), $arNavigation)))
{
    ...
    if(stripos($APPLICATION->GetCurDir(), '/katalog_tovarov/vazy/') !== false && isset($_REQUEST['form_submit'])) {
        //$GLOBALS['arrFilter'] = array('>=PROPERTY_VYSOTA_TOVARA' => $_REQUEST['slider_filterleft'], '<=PROPERTY_VYSOTA_TOVARA' => $_REQUEST['slider_filterright']);    
        //$arAmount = explode("-", $_REQUEST['amount']);
        
        //$arrFilter = array_merge($arrFilter, array('>=PROPERTY_VYSOTA_TOVARA' => intval($arAmount[0]), '<=PROPERTY_VYSOTA_TOVARA' => intval($arAmount[1])));
    }
    ...
    //if($USER->IsAdmin()) {
        if(stripos($APPLICATION->GetCurDir(), '/katalog_tovarov/vazy/') !== false) {
            ...
            else if(isset($_REQUEST['del_filter']) || (!isset($_REQUEST['slider_filterleft']) && !isset($_REQUEST['slider_filterright']))) {
                unset($_SESSION['arrFilter']);
            }

            if(isset($_SESSION['arrFilter'])) {
                $arrFilter = array_merge($arrFilter, $_SESSION['arrFilter']);
            } */
            
                if(isset($_REQUEST['custom_filter'])) {
                    unset($_SESSION['arrFilter']);
                    foreach($_REQUEST as $key => $value) {                    
                        if(stripos($key, 'arrFilter') !== false) {                            
                            $_SESSION['arrFilter'][] = $key;
                        }
                    }
                }
                ...
                if(isset($_SESSION['arrFilter']) && isset($_REQUEST['high_filter'])) {
                    $addUrl = "";
                    foreach($_SESSION['arrFilter'] as $filter) {
                        $addUrl .= $filter.'=Y&';
                    }
                    
                    $url = (stripos($APPLICATION->GetCurUri(), '?') !== false) ? $APPLICATION->GetCurPageParam("", array("high_filter"))."&".$addUrl."set_filter=Y&amount=".$_REQUEST['amount'] : $APPLICATION->GetCurPageParam("", array("high_filter"))."?".$addUrl."set_filter=Y".$_REQUEST['amount'];
                    LocalRedirect($url);
                }
