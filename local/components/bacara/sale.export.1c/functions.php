...
*/
	function GetFullDeliveryAddress($arOrderId)
	{
        ...
        while ($arOrderPropVals = $dbOrderPropVals->Fetch())
        {
            if ($arOrderPropVals["PROP_TYPE"] == "LOCATION")
            {
                ...
                $address .= ($arAddress['COUNTRY_NAME']) ? $arAddress['COUNTRY_NAME'].', ' : '';
                $address .= ($arAddress['REGION_NAME']) ? $arAddress['REGION_NAME'].', ' : '';
                $address .= ($arAddress['CITY_NAME']) ? $arAddress['CITY_NAME'].', ' : '';
            }
            else {
    ...
    {
        GLOBAL $USER;
        $rsUser = $USER->GetList(($by = "ID"), ($order = "desc"), array("ID" => $userId), array("SELECT" => array("UF_*")));

        $arUser = $rsUser->GetNext(true, false);

        $docsCount = count($arUser['UF_DOCS']);

        if ($docsCount == 1) {
            $idDoc = $arUser['UF_DOCS'][0];
            $rsFile = CFile::GetByID($idDoc);
            $arFile[] = $rsFile->Fetch();
        } else if ($docsCount > 1) {
            foreach ($arUser['UF_DOCS'] as $doc) {
                $rsFile = CFile::GetByID($doc);
                $arFile[] = $rsFile->Fetch();
            }
        }
    ...
    function GetNewID($userId)
    {
        ...
        $rsUser = $USER->GetList(($by = "ID"), ($order = "desc"), array("ID" => $userId), array("FIELDS" => array("ID"), "SELECT" => array("UF_1C_ID")));

        $arUser = $rsUser->GetNext(true, false);

		//AddMessage2Log($arUser['UF_1C_ID']);
    ...
    function ExportOrders2Xml($arFilter = Array(), $nTopCount = 0, $currency = "", $crmMode = false, $time_limit = 0, $version = false, $arOptions = Array())
    {
        ...
        while ($arOrder = $dbOrderList->Fetch()) {
            ...
            $deliveryAdr = CSaleExport2XML::ExportContragents(
            ...
            echo htmlspecialcharsbx("[" . $arOrder["STATUS_ID"] . "] " . $arStatus["NAME"]); ?></<?= GetMessage("SALE_EXPORT_VALUE") ?>>
            </<?= GetMessage("SALE_EXPORT_PROPERTY_VALUE") ?>>
            <<?= GetMessage("SALE_EXPORT_PROPERTY_VALUE") ?>>
    ...
    function ExportContragents($arOrder = array(), $arProp = array(), $agent = array(), &$arResultStat, $bNewVersion = false, $arOptions = array(),$arProps)
    {
        $bExportFromCrm = (isset($arOptions["EXPORT_FROM_CRM"]) && $arOptions["EXPORT_FROM_CRM"] === "Y");
        ?>
        ...
        <<?= 'НазваниеКомпанииКраткое' ?>><?= $userInfo['UF_COMPANY_SHORT'] ?></<?= 'НазваниеКомпанииКраткое' ?>>
        <<?= 'ИНН' ?>><?= $userInfo['UF_INN'] ?></<?= 'ИНН' ?>>
        <<?= 'КПП' ?>><?= $userInfo['UF_KPP'] ?></<?= 'КПП' ?>>
        <<?= 'ОГРН' ?>><?= $userInfo['UF_OGRN'] ?></<?= 'ОГРН' ?>>
        ...
        <<?= 'НомерПаспорта' ?>><?= $arProps['9']?$arProps['9']:use ?></<?= 'НомерПаспорта' ?>>
